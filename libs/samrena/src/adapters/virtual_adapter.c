...
static void virtual_dump_stats(void *context, FILE *out) {
  ...
  fprintf(out, "Reserved size: %lu bytes (%.2f MB)\n", ctx->reserved_size,
          ctx->reserved_size / (1024.0 * 1024.0));
  fprintf(out, "Committed size: %lu bytes (%.2f MB)\n", ctx->committed_size,
          ctx->committed_size / (1024.0 * 1024.0));
  fprintf(out, "Allocated size: %lu bytes (%.2f MB)\n", ctx->allocated_size,
          ctx->allocated_size / (1024.0 * 1024.0));
...
static void virtual_prefetch(void *context, uint64_t expected_size) {
  ...
  if (needed_committed > ctx->committed_size) {
    ...
    to_commit = ((to_commit + ctx->commit_granularity - 1) / ctx->commit_granularity) *
                ctx->commit_granularity;
...
#ifdef _WIN32
  WIN32_MEMORY_RANGE_ENTRY entry = {.VirtualAddress = ctx->base_address,
...
const SamrenaOps virtual_adapter_ops = {.name = "virtual",
