...
#define SAMRENA_HAS_OP(arena, op) ((arena)->impl->ops->op != NULL)

#define SAMRENA_CALL_OP(arena, op, ...)                                                            \
  (SAMRENA_HAS_OP(arena, op) ? (arena)->impl->ops->op((arena)->context, ##__VA_ARGS__)             \
...
#define SAMRENA_CALL_OP_PTR(arena, op, ...)                                                        \
  (SAMRENA_HAS_OP(arena, op) ? (arena)->impl->ops->op((arena)->context, ##__VA_ARGS__) : NULL)

#define SAMRENA_CALL_OP_UINT64(arena, op, ...)                                                     \
  (SAMRENA_HAS_OP(arena, op) ? (arena)->impl->ops->op((arena)->context, ##__VA_ARGS__) : 0)
