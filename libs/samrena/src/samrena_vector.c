...
static size_t calculate_arena_size(size_t element_size, size_t initial_capacity) {
  ...
  if (with_growth < min_arena_size)
    return min_arena_size;
  if (with_growth > max_arena_size)
    return max_arena_size;

  size_t rounded = 1;
  while (rounded < with_growth)
    rounded <<= 1;
...
size_t samrena_vector_size(const SamrenaVector *vec) { return vec ? vec->size : 0; }

size_t samrena_vector_capacity(const SamrenaVector *vec) { return vec ? vec->capacity : 0; }

bool samrena_vector_is_empty(const SamrenaVector *vec) { return !vec || vec->size == 0; }

bool samrena_vector_is_full(const SamrenaVector *vec) { return vec && vec->size >= vec->capacity; }
