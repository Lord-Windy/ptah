...
void test_similar_keys() {
  ...
  const char *similar_keys[] = {"abc", "acb",   "bac",   "bca",   "cab",
                                "cba", "test1", "test2", "test3", "test4"};
...
void test_collision_chain_integrity() {
  ...
  const char *test_keys[] = {"first", "second", "third", "fourth", "fifth"};
...
void test_resize_preserves_data() {
  ...
  const char *test_data[][2] = {{"apple", "fruit"},        {"carrot", "vegetable"},
                                {"salmon", "fish"},        {"bread", "grain"},
                                {"milk", "dairy"},         {"chicken", "meat"},
                                {"broccoli", "vegetable"}, {"banana", "fruit"}};
...
void benchmark_mixed_operations() {
  ...
  printf("Collision rate: %.3f%%\n", stats.total_operations > 0
                                         ? (100.0 * stats.total_collisions) / stats.total_operations
                                         : 0.0);
...
void test_collision_visualization() {
  ...
  printf("Capacity: %zu, Size: %zu, Load factor: %.3f\n", comb->capacity, comb->size,
         (float)comb->size / comb->capacity);
  printf("\n");
