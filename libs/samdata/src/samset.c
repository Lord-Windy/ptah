...
SamSet *samset_create_with_hash(size_t element_size, size_t initial_capacity, 
    ...
    return samset_create_custom(element_size, initial_capacity, samrena,
                              samset_get_hash_function(hash_func),
                              samset_default_equals);
}
...
SamSetError samset_get_last_error(const SamSet *samset) {
    if (samset == NULL) return SAMSET_ERROR_NULL_PARAM;
...
bool samset_add(SamSet *samset, const void *element) {
    if (samset == NULL || element == NULL) {
        if (samset) samset_set_error(samset, SAMSET_ERROR_NULL_PARAM);
        return false;
    }
    ...
    while (current != NULL) {
        ...
        if (current->hash == hash && 
            samset->equals(current->element, element, samset->element_size)) {
...
size_t samset_size(const SamSet *samset) {
    if (samset == NULL) return 0;
    return samset->size;
}

bool samset_is_empty(const SamSet *samset) {
    if (samset == NULL) return true;
...
SamSetStats samset_get_stats(const SamSet *samset) {
    ...
    if (samset == NULL) return empty_stats;
...
void samset_foreach(const SamSet *samset, SamSetIterator iterator, void *user_data) {
    if (samset == NULL || iterator == NULL) return;
...
SamSet *samset_copy(const SamSet *samset, Samrena *samrena) {
    ...
    SamSet *copy = samset_create_custom(samset->element_size, samset->capacity, samrena,
