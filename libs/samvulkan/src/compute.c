...
SamVulkanCompute *samvulkan_compute_create(SamVulkanDevice *device) {
  ...
  compute->queue_family_index =
      samvulkan_device_get_queue_family_index(device, SAMVULKAN_QUEUE_COMPUTE);

  VkCommandPoolCreateInfo pool_info = {.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO,
                                       .flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT,
                                       .queueFamilyIndex = compute->queue_family_index};

  if (vkCreateCommandPool(compute->vk_device, &pool_info, NULL, &compute->command_pool) !=
      VK_SUCCESS) {
    free(compute);
    return NULL;
  }
  ...
  if (vkAllocateCommandBuffers(compute->vk_device, &alloc_info, &compute->command_buffer) !=
      VK_SUCCESS) {
...
bool samvulkan_compute_create_pipeline(SamVulkanCompute *compute,
  ...
  if (vkCreateDescriptorSetLayout(compute->vk_device, &layout_info, NULL,
                                  &compute->descriptor_set_layout) != VK_SUCCESS) {
    vkDestroyShaderModule(compute->vk_device, shader_module, NULL);
    return false;
  }

  VkPushConstantRange push_constant = {.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT,
                                       .offset = 0,
  ...
  if (vkCreatePipelineLayout(compute->vk_device, &pipeline_layout_info, NULL,
  ...
  VkResult result = vkCreateComputePipelines(compute->vk_device, VK_NULL_HANDLE, 1, &pipeline_info,
                                             NULL, &compute->compute_pipeline);
  ...
  if (vkCreateDescriptorPool(compute->vk_device, &pool_info, NULL, &compute->descriptor_pool) !=
      VK_SUCCESS) {
    return false;
  }
  ...
  if (vkAllocateDescriptorSets(compute->vk_device, &alloc_info, &compute->descriptor_set) !=
      VK_SUCCESS) {
    return false;
  }
...
SamVulkanBuffer *samvulkan_compute_create_buffer(SamVulkanCompute *compute, size_t size,
  ...
  if (usage & SAMVULKAN_BUFFER_STORAGE)
    vk_usage |= VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
  if (usage & SAMVULKAN_BUFFER_UNIFORM)
    vk_usage |= VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
  if (usage & SAMVULKAN_BUFFER_TRANSFER_SRC)
    vk_usage |= VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
  if (usage & SAMVULKAN_BUFFER_TRANSFER_DST)
  ...
  if (properties & SAMVULKAN_MEMORY_DEVICE_LOCAL)
    vk_properties |= VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
  if (properties & SAMVULKAN_MEMORY_HOST_VISIBLE)
    vk_properties |= VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT;
  if (properties & SAMVULKAN_MEMORY_HOST_COHERENT)
    vk_properties |= VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;

  uint32_t memory_type =
      find_memory_type(compute->physical_device, mem_requirements.memoryTypeBits, vk_properties);
...
void *samvulkan_compute_map_buffer(SamVulkanBuffer *buffer) {
  ...
  if (vkMapMemory(buffer->compute->vk_device, buffer->memory, 0, buffer->size, 0,
                  &buffer->mapped_data) != VK_SUCCESS) {
    return NULL;
  }
...
void samvulkan_compute_bind_buffer(SamVulkanCompute *compute, uint32_t binding,
  ...
  if (!compute || !buffer || !compute->descriptor_set)
    return;

  VkDescriptorBufferInfo buffer_info = {.buffer = buffer->buffer,
                                        .offset = 0,
                                        .range = buffer->size};
...
void samvulkan_compute_set_push_constants(SamVulkanCompute *compute, const void *data,
                                          size_t size) {
  if (!compute || !data || size > compute->push_constant_size)
    return;

  vkCmdPushConstants(compute->command_buffer, compute->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT,
                     0, (uint32_t)size, data);
}
...
bool samvulkan_compute_dispatch(SamVulkanCompute *compute, uint32_t x, uint32_t y, uint32_t z) {
  ...
  vkCmdBindDescriptorSets(compute->command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                          compute->pipeline_layout, 0, 1, &compute->descriptor_set, 0, NULL);
