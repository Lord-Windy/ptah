...
SamVulkanInstance* samvulkan_instance_create(const SamVulkanInstanceConfig* config) {
    ...
    if (config->enable_validation) {
        ...
        if (vkCreateDebugUtilsMessengerEXT) {
            vkCreateDebugUtilsMessengerEXT(instance->instance, &debug_create_info, NULL, &instance->debug_messenger);
        }
    }
    ...
    if (instance->physical_device_count > 0) {
        ...
        vkEnumeratePhysicalDevices(instance->instance, &instance->physical_device_count, instance->physical_devices);
    }
