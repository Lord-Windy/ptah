...
- **`lib/`**: Contains library crates (reusable components)
  - **ptah-lib** (`lib/ptah-lib/`): Core library providing shared functionality
    - Currently exports `greet()` and `add()` functions
    - Contains unit tests for all public functions
...
The workspace is configured in the root `Cargo.toml` with shared package metadata (authors, edition, license, rust-version) that all member crates inherit.