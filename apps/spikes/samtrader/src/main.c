...
void debug_fetch_and_print_ohlcv(SamtraderDb *db, const char *code, const char *exchange,
  ...
  printf("\nFetching %s data from %s exchange for %d-%d (%d years)...\n\n", code, exchange,
         start_year, start_year + num_years - 1, num_years);
  ...
  if (ohlcv_data) {
    ...
    if (total_records > records_to_show) {
      ...
      SamrenaVector *slice =
          samrena_vector_slice(ohlcv_data, start_index, total_records, db->arena);
      if (slice) {
