...
int main() {
  ...
  SamrenaVector *doubled_vec =
      samrena_vector_map(numbers->_vec, sizeof(int), double_int_transform, NULL, arena);
  if (doubled_vec) {
  ...
  int threshold = 5;
  SamrenaVector *filtered_vec =
      samrena_vector_filter(numbers->_vec, is_greater_than_threshold, &threshold, arena);
  if (filtered_vec) {
  ...
  if (even_vec) {
    SamrenaVector *chained_vec =
        samrena_vector_map(even_vec, sizeof(int), double_int_transform, NULL, arena);
    if (chained_vec) {
      printf("   Result: ");
