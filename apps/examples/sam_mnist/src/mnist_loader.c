...
static bool grow_dataset_capacity(MnistDataset *dataset, size_t min_capacity) {
  if (dataset->capacity >= min_capacity)
    return true;
  ...
  MnistSample *new_samples =
      (MnistSample *)samrena_push(dataset->arena, new_capacity * sizeof(MnistSample));
  if (!new_samples)
    return false;
...
void mnist_get_batch(const MnistDataset *dataset, size_t start, size_t batch_size,
  ...
  if (!dataset || !batch_out || !actual_size)
    return;
...
void mnist_shuffle(MnistDataset *dataset) {
  if (!dataset || dataset->num_samples <= 1)
    return;
