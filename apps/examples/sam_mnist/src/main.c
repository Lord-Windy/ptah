...
int main(int argc, char **argv) {
  ...
  for (uint64_t i = 0; i < config.hidden_layer_count; i++) {
    ...
    if (i < config.hidden_layer_count - 1)
      printf(", ");
  }
  printf(")\n");
  ...
  float *training_inputs =
      samrena_push(arena, sizeof(float) * samples_per_epoch * MNIST_IMAGE_SIZE);
  float *training_targets =
      samrena_push(arena, sizeof(float) * samples_per_epoch * MNIST_NUM_CLASSES);
  ...
  for (size_t i = 0; i < samples_per_epoch; i++) {
    // Copy input pixels
    memcpy(&training_inputs[i * MNIST_IMAGE_SIZE], train_data->samples[i].pixels,
           sizeof(float) * MNIST_IMAGE_SIZE);

    // Convert label to one-hot encoding
    mnist_label_to_onehot(train_data->samples[i].label, &training_targets[i * MNIST_NUM_CLASSES]);
  }

  SamNeuralSamples training_samples = {.inputs = training_inputs,
  ...
  float *testing_inputs =
      samrena_push(arena, sizeof(float) * test_samples_count * MNIST_IMAGE_SIZE);
  float *testing_targets =
      samrena_push(arena, sizeof(float) * test_samples_count * MNIST_NUM_CLASSES);
  ...
  for (size_t i = 0; i < test_samples_count; i++) {
    memcpy(&testing_inputs[i * MNIST_IMAGE_SIZE], test_data->samples[i].pixels,
           sizeof(float) * MNIST_IMAGE_SIZE);

    mnist_label_to_onehot(test_data->samples[i].label, &testing_targets[i * MNIST_NUM_CLASSES]);
  }

  SamNeuralSamples testing_samples = {.inputs = testing_inputs,
