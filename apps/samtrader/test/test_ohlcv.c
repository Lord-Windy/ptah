...
static int test_ohlcv_create(void) {
    ...
    SamtraderOhlcv *ohlcv = samtrader_ohlcv_create(
        arena, "AAPL", "US", 1704067200, 150.0, 155.0, 149.0, 153.0, 1000000);
...
static int test_ohlcv_vector(void) {
    ...
    const SamtraderOhlcv *retrieved =
        (const SamtraderOhlcv *)samrena_vector_at_const(vec, 0);
...
static int test_indicator_series_create(void) {
    ...
    ASSERT(samtrader_indicator_series_size(series) == 0,
...
static int test_indicator_series_add_simple(void) {
    ...
    for (int i = 0; i < 14; i++) {
        SamtraderIndicatorValue *val = samtrader_indicator_add_simple(
            series, 1704067200 + (i * 86400), 0.0, false);
    ...
    for (int i = 14; i < 30; i++) {
        ...
        SamtraderIndicatorValue *val = samtrader_indicator_add_simple(
            series, 1704067200 + (i * 86400), rsi_value, true);
        ASSERT(val != NULL, "Failed to add value");
    }

    ASSERT(samtrader_indicator_series_size(series) == 30,
           "Series size should be 30");

    const SamtraderIndicatorValue *val =
        samtrader_indicator_series_at(series, 0);
...
static int test_macd_series(void) {
    ...
    SamtraderIndicatorSeries *series =
        samtrader_macd_series_create(arena, 12, 26, 9, 100);
...
static int test_bollinger_series(void) {
    ...
    SamtraderIndicatorSeries *series =
        samtrader_bollinger_series_create(arena, 20, 2.0, 100);
    ...
    SamtraderIndicatorValue *val = samtrader_indicator_add_bollinger(
        series, 1704067200, 160.0, 150.0, 140.0, true);
...
static int test_stochastic_series(void) {
    ...
    SamtraderIndicatorSeries *series =
        samtrader_stochastic_series_create(arena, 14, 3, 100);

    ASSERT(series != NULL, "Failed to create Stochastic series");
    ASSERT(series->type == SAMTRADER_IND_STOCHASTIC,
           "Type should be STOCHASTIC");
...
static int test_pivot_series(void) {
    ...
    SamtraderIndicatorSeries *series =
        samtrader_pivot_series_create(arena, 100);

    ASSERT(series != NULL, "Failed to create Pivot series");
    ASSERT(series->type == SAMTRADER_IND_PIVOT, "Type should be PIVOT");

    SamtraderIndicatorValue *val = samtrader_indicator_add_pivot(
        series, 1704067200, 150.0, 155.0, 160.0, 165.0, 145.0, 140.0, 135.0,
        true);
...
static int test_indicator_type_name(void) {
    ...
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_SMA), "SMA") == 0,
           "SMA name");
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_EMA), "EMA") == 0,
           "EMA name");
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_RSI), "RSI") == 0,
           "RSI name");
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_MACD), "MACD") ==
               0,
           "MACD name");
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_BOLLINGER),
                  "Bollinger") == 0,
           "Bollinger name");
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_ATR), "ATR") == 0,
           "ATR name");
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_STOCHASTIC),
                  "Stochastic") == 0,
           "Stochastic name");
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_PIVOT),
                  "Pivot") == 0,
           "Pivot name");
...
static int test_type_mismatch_rejection(void) {
    ...
    val = samtrader_indicator_add_bollinger(sma_series, 1704067200, 160.0,
                                            150.0, 140.0, true);
...
int main(void) {
    printf("=== OHLCV and Indicator Data Structures Tests ===\n\n");
