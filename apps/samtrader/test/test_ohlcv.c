...
static int test_indicator_series_create(void) {
    ...
    ASSERT(series->period == 20, "Period mismatch");
...
static int test_indicator_series_add(void) {
    printf("Testing samtrader_indicator_series_add...\n");
    ...
    for (int i = 0; i < 14; i++) {
        SamtraderIndicatorValue *val = samtrader_indicator_series_add(
    ...
    ASSERT_DOUBLE_EQ(val->value, 50.0, "Value at index 14");

    double latest;
    bool found = samtrader_indicator_series_latest(series, &latest);
...
static int test_indicator_type_name(void) {
    ...
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_MACD), "MACD") ==
               0,
           "MACD name");
    ASSERT(strcmp(samtrader_indicator_type_name(SAMTRADER_IND_BOLLINGER_UPPER),
                  "Bollinger Upper") == 0,
           "Bollinger Upper name");
...
int main(void) {
    ...
    failures += test_indicator_series_add();
